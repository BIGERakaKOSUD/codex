...
function EditableCell({
  ...
  field: string;
  editable: boolean;
  onSave: (row: Row, field: string, value: string | boolean) => void;
}) {
...
export function EconomicsDashboard() {
  ...
  async function loadRows() {
    ...
    setError("");
    try {
      const response = await fetch(`/api/economics?${query}`);
  ...
  async function saveCell(row: Row, field: string, value: string | boolean) {
  ...
  async function importFile(file: File) {
    ...
    await runAction("Импорт", () => fetch("/api/import/manual", { method: "POST", body: formData }));
  ...
  const metrics = useMemo(() => {
    ...
    return [
      ...
      ["Нет себестоимости", filteredRows.filter((row) => row.sourceMap.cost_price === "missing").length],
      ["Нет объёма", filteredRows.filter((row) => row.sourceMap.product_volume_liters === "missing").length],
      ["Рекламные расходы", sum("ad_cost_per_buyout_rub")],
  ...
  return (
    <section className="page">
      <div className="toolbar">
        ...
        <button className="btn primary" disabled={loading} onClick={() => runAction("Синхронизация", () => fetch("/api/ozon/products/sync", { method: "POST" }))}>
          <RefreshCcw size={16} />
          Синхронизировать товары из Ozon
        </button>
        <button className="btn" disabled={loading} onClick={() => runAction("Финансы", () => fetch("/api/ozon/finance/load", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ from, to }) }))}>
          <RotateCw size={16} />
          Загрузить финансовые операции
        </button>
        <label className="file-label">
          <FileSpreadsheet size={16} />
          Импорт ручных данных из Excel/CSV
        ...
        <a className="btn" href={`/api/economics?${query}&format=xlsx`}>
          <Download size={16} />
          Экспорт в Excel
        </a>
      </div>
      ...
      <div className="filters">
        ...
        <label><input type="checkbox" checked={missingVolumeOnly} onChange={(event) => setMissingVolumeOnly(event.target.checked)} /> нет объёма</label>
